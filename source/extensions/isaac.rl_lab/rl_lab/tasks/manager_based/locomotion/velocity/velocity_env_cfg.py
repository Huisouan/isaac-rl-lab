...
class RewardsCfg:
    ...
    base_height_l2 = RewTerm(func=mdp.base_height_l2,weight=0,params={"asset_cfg": SceneEntityCfg("robot", body_names="base"), "target_height": 0.0},)
    body_lin_acc_l2 = RewTerm(func=mdp.body_lin_acc_l2,weight=0,params={"asset_cfg": SceneEntityCfg("robot", body_names="base")},)
    # Joint penaltie
    ...
    # Action penalties
    applied_torque_limits = RewTerm(func=mdp.applied_torque_limits,weight=0,arams={"asset_cfg": SceneEntityCfg("robot", joint_names=".*")},)
    action_rate_l2 = RewTerm(func=mdp.action_rate_l2, weight=-0.01)

    # Contact sensor
    undesired_contacts = RewTerm(func=mdp.undesired_contacts,weight=0,params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*THIGH"), "threshold": 1.0},)
    contact_forces = RewTerm(func=mdp.contact_forces,weight=0,params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*FOOT"), "threshold": 1.0},)

    # Velocity-tracking rewards
    track_lin_vel_xy_exp = RewTerm(func=mdp.track_lin_vel_xy_exp, weight=0, params={"command_name": "base_velocity", "std": math.sqrt(0.25)})
    track_ang_vel_z_exp = RewTerm(func=mdp.track_ang_vel_z_exp, weight=0, params={"command_name": "base_velocity", "std": math.sqrt(0.25)})

    # Others
    feet_air_time = RewTerm(func=mdp.feet_air_time,weight=0,
        params={
            "command_name": "base_velocity",
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*FOOT"),
            "threshold": 0.5,},)

    foot_contact = RewTerm(func=mdp.foot_contact,weight=0,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*FOOT"),
            "command_name": "base_velocity",
            "expect_contact_num": 2,},)

    base_height_rough_l2 = RewTerm(func=mdp.base_height_rough_l2,weight=0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
    ...
    stand_still_when_zero_command = RewTerm(
        func=mdp.stand_still_when_zero_command,weight=0,
        params={"command_name": "base_velocity"},)
