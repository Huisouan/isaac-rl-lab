...
# reference motion
FRAME_DURATION = 0.008333333333333333
...
REF_NECK_JOINT_ID = 47

REF_HIP_JOINT_IDS = [58,49,13,2]
REF_TOE_JOINT_IDS = [63,54,19,8]

chain_foot_fl = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.FL_FOOT_NAME
)
chain_foot_fr = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.FR_FOOT_NAME
)
chain_foot_rl = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.HL_FOOT_NAME
)
chain_foot_rr = build_serial_chain_from_urdf(
    open(config.URDF_FILENAME, 'rb').read(), config.HR_FOOT_NAME
)
...
def retarget_pose(robot, default_pose, ref_joint_pos):
    ...
    参数:
    - robot: pybullet中的机器人对象。
    - default_pose: 默认的关节姿势，用作IK计算的休息姿势。
    - ref_joint_pos: 参考机器人的关节位置字典。

    返回:
    - pose: 调整后的机器人姿态，包括根节点位置、根节点旋转和关节姿势。
    """
...
def generate_motion_list(folder_path):
    """
    从指定文件夹中的CSV文件生成mocap_motions列表。

    :param folder_path: 包含CSV文件的文件夹路径
    :return: mocap_motions列表
    """
    mocap_motions = []
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv'):
            ...
            motion_name = '_'.join(parts[1:-1])  # 保留中间的部分，例如 "back_001"
            
            # 构造完整的文件路径
            file_path = os.path.join(folder_path, filename)
            
            # 添加到列表
            mocap_motions.append([motion_name, file_path])

    return mocap_motions


def load_csvref_data(JOINT_POS_FILENAME, FRAME_START=None, FRAME_END=None, scaler=0.008):
    """
    从 CSV 文件加载关节位置数据，并根据参数进行裁剪和缩放。

    :param JOINT_POS_FILENAME: CSV 文件路径。
    :param FRAME_START: 起始帧索引，默认为 None，表示从第一帧开始。
    :param FRAME_END: 结束帧索引，默认为 None，表示到最后一帧结束。
    :param scaler: 缩放因子，默认为 1.0。
    :return: 处理后的关节位置数据数组以及点的名字列表。
    """
    ...
    # 打开 CSV 文件
    with open(JOINT_POS_FILENAME, mode='r', newline='') as file:
        ...
        # 提取点的名字
        point_names = [header[i] for i in range(0, len(header), 3)]

        # 逐行读取数据
        for row in reader:
            # 将每行数据转换为浮点数并添加到列表中
            joint_pos_data.append([float(value) for value in row])

    # 转换为 NumPy 数组
    joint_pos_data = np.array(joint_pos_data)
    ...
    # 进行缩放
    joint_pos_data *= scaler  # 将每个元素乘以缩放因子

    return joint_pos_data, point_names
...
def retarget_motion(robot, joint_pos_data):
    ...
    该函数根据给定的关节位置数据计算机器人的运动数据。它处理每一帧，生成新的姿态数据，
    包括线速度、角速度、关节速度和脚趾速度。

    参数:
    - robot: 要重新定位运动的机器人对象。
    - joint_pos_data: 包含一系列帧的关节位置数据的 numpy 数组。

    返回:
    - new_frames: 包含调整后的机器人运动数据的 numpy 数组。
    """
    # 计算总帧数
...
def main(argv):
    ...
    mocapmotions = generate_motion_list(config.MOCAP_DIR)
    for mocap_motion in mocapmotions:
        ...
        # 读取每一行为一个数组
        joint_pos_data,joint_pos_names = load_csvref_data(mocap_motion[1])
        ...
        output_motion(retarget_frames, output_file, 1, FRAME_DURATION)
